...
from utils.UV_Predictor import get_predictor
...
QC_PREDICTOR = os.path.join(DIR_NAME, 'models', 'qc.pth')
AGE_PREDICTOR = os.path.join(DIR_NAME, 'models', 'age.pth')
UV_PREDICTOR = os.path.join(DIR_NAME, 'models', 'uv.pth')
...
def cno_detection(source, kde_dir, conf, cno_model, file_list, model_type):
    ...
    qc_predictor = get_predictor(QC_PREDICTOR, model_name='RETFound_mae', num_classes=2, input_size=224)
    age_predictor = get_predictor(AGE_PREDICTOR, model_name='RETFound_mae', num_classes=2, input_size=224)
    uv_predictor = get_predictor(UV_PREDICTOR, model_name='RETFound_mae', num_classes=2, input_size=224)
    ...
    for image_path in png_files:
        ...
        result = qc_predictor.predict(image_path, predictor_type="qc")
    ...
    for image_path in png_files:
        print(f"\nAge Processing: {image_path.name}")
        result = age_predictor.predict(image_path, predictor_type="age")

        print(f"Predicted class: {result['predicted_class']}")
        print(f"Result: {result['result']}")
        print(f"Confidence: {result['confidence']:.4f}")
        print(f"All probabilities: {result['probabilities']}")
        age_pred.append(result['result'])
        age_conf.append(round(result['confidence'], 3))
    
    for image_path in png_files:
        print(f"\nUV Processing: {image_path.name}")
        result = uv_predictor.predict(image_path, predictor_type="uv")

        print(f"Predicted class: {result['predicted_class']}")
        print(f"Result: {result['result']}")
        print(f"Confidence: {result['confidence']:.4f}")
        print(f"All probabilities: {result['probabilities']}")
        uv_pred.append(result['result'])
        uv_conf.append(round(result['confidence'], 3))

    return cno_col, avg_area_col, total_area_col, total_layer_area, total_layer_cno, total_layer_density, qc_pred, qc_conf, age_pred, age_conf, uv_pred, uv_conf
...
def cno_detect(folder_dir, model, conf):
    ...
    # Make Function
    cno_col, avg_area_col, total_area_col, layer_area, layer_cno, layer_density, qc_prediction, qc_conf, age_prediction, age_conf, uv_prediction, uv_conf = cno_detection(enhanced_png_path,
                                                                                                kde_png_path,
                                                                                                conf, CNO_model,
                                                                                                file_list, model)
