...
class ModelPredictor:
    ...
    def _preprocess_image(self, image_path):
        ...
        transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.819, 0.444, 0.118],
                                 std=[0.204, 0.291, 0.173])
        ])
    ...
    def predict(self, image_path, predictor_type="qc"):
        """
        Perform prediction on a single image and return results.

        predictor_type:
            - "qc"  → QC Predictor (Passed / Failed)
            - "age" → Age Predictor (0-29, 30-49, 50+)
            - "uv"  → UV Predictor (Low UV / High UV)
        """
        ...
        # Map predictions based on predictor type
        if predictor_type == "qc":
            class_map = {0: "Failed", 1: "Passed"}
        elif predictor_type == "age":
            class_map = {0: "0-29", 1: "30-49", 2: "50+"}
        elif predictor_type == "uv":
            class_map = {0: "Low UV", 1: "High UV"}
        else:
