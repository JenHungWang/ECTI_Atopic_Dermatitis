...
# Folder Path
AFM_Sample = "TW_G4_TL0_T4_No.05"
ROOT = '/home/jenhung/PycharmProjects/AFM_Sample/test/' + AFM_Sample + '/'
CSV_PATH = os.path.join(ROOT, "CNO_Detection")
GROWCUT_PATH = os.path.join(ROOT, "CNO_Detection/GrowCut")
ORIGINAL_PNG_PATH = os.path.join(ROOT, "CNO_Detection/Image/Original")
ENHANCED_PNG_PATH = os.path.join(ROOT, "CNO_Detection/Image/Enhanced")
...
try:
    ...
    print("Directory '%s' can not be created" % GROWCUT_PATH)

# Model Path
YOLO_MODEL_PATH = '/home/jenhung/PycharmProjects/bcr_img/Models/CNO_Detection_b4_m.pt'
file_list = []
growcut_list = []
yolo_list = []
...
def treat_one_image(fn, outdir=ROOT):
    # load data
    im = load_im(fn)
    ...
    growcut_list.append(ct)


def main():
    encyc = []
    sourceroot = ROOT
    walk = os.walk(sourceroot)
    for d, sd, files in walk:
        for fn in files:
            if fn[-10:].lower() == '_trace.bcr':
                encyc.append(d + '/' + fn)
    encyc.sort()
    ...
    results = model.predict(source, project=CSV_PATH, name="YOLO", save=True, show_labels=False, show_conf=False,
                            line_width=1, iou=0.5, conf=0.2, save_txt=True, max_det=1200)

    for result in results:
        CNO = len(result.boxes)
        yolo_list.append(CNO)

    # Write CSV
    # open the file in the write mode
    f = open(CSV_PATH + '/CNO.csv', 'w')
    header = ['File', 'GrowCut', 'YOLOv8']
    ...
    for i in range(len(file_list)):
        data = [file_list[i], growcut_list[i], yolo_list[i]]
        # print(data)
