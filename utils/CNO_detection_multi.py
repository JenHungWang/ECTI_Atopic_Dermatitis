...
# Folder Path
ROOT = '/home/jenhung/PycharmProjects/AFM_Sample/test/'
...
folder_list = []
yolo_list = []
for folderName in glob.glob(ROOT + '*'):
...
def main():
    ...
    for folder in folder_list:
        ...
        file_list = []
        growcut_list = []
        sourceroot = os.path.join(ROOT, folder)
        csv_path = os.path.join(sourceroot, "CNO_Detection")
        growcut_path = os.path.join(sourceroot, "CNO_Detection/GrowCut")
        original_png_path = os.path.join(sourceroot, "CNO_Detection/Image/Original")
        enhanced_png_path = os.path.join(sourceroot, "CNO_Detection/Image/Enhanced")
        ...
        for d, sd, files in walk:
            directory = d.split('/')[-1]
            for fn in files:
                if fn[-10:].lower() == '_trace.bcr' and directory != 'MAPS' and directory != 'Extras':
                    encyc.append(d + '/' + fn)
        encyc.sort()
